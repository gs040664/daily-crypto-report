...
2. 5 個幣種的【行動指南】 (排版需乾淨俐落)：
   - 每個幣種只需顯示：「現價」與「具體交易策略」。
   - 策略內容請直接給出：『目前強弱狀態』、『建議入場價位與必須確認的價格行為』、『預期止盈與止損價位』。
   - ⚠️ 嚴格限制：絕對不要羅列生硬的技術數據！不需要把 EMA、MACD、RSI 等數字一一寫出來。除非某個數據是支撐你這個策略的「前三大核心依據」(例如： POC 剛好形成強支撐、或 RSI 出現嚴重背離)，否則一律省略不講，直接給結論！
   - 策略思路請自然融合 TiaBTC 的最新觀點。
3. 文末附上參考影片連結：{video_url}
4. 語氣果斷專業、像個身經百戰的操盤手。直接輸出純文本，不要加上 ``` 區塊包裝，且總字數必須控制在 1500 字以內，確保 Discord 能一次發送完畢。
"""
